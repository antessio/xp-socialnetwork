...
public class CommandService {
    ...
    public CommandService(){
        ...
        this.userWallDAO = new UserWallDAO();
    }
    public CommandService(UserPostDAO userPostDAO, UserDAO userDAO,UserFollowerDAO userFollowerDAO,UserWallDAO userWallDAO) {
    ...
    public String handle(String command) throws ServiceException{
        ...
        if(insertPostMatcher.matches()) {
            ...
            userFollowerDAO.insert(username,follower);
        }else if(wallPattern.matches()){
            String username = wallPattern.group(1);
            userDAO.find(username).orElseThrow(()->new ServiceException(username+" not found"));
            return collectUserPosts(userWallDAO.get(username).getPosts(), true);
        }
        return "";
    }
